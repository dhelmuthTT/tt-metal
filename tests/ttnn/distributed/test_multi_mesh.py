...
# Simple TTNN example demonstarting the ability to interface with multiple MeshDevices across multiple processes.
...
def run_multiprocess_workload():
    ...
    if not ttnn.distributed_context.is_initialized():
        raise ValueError("Distributed context not initialized")
    if int(ttnn.distributed_context.get_size()) != 2:
    ...
    if int(ttnn.distributed_context.get_rank()) == 0:
        # Create send socket, run the first op on the inpute tensor and forward the result to the receiver
